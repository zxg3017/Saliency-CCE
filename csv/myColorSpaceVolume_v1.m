function [csv,csv_refined] = myColorSpaceVolume_v1(img)
...
L_a=(L).*a;
csv_Lab=(1.0/2.0)*pi*L_a;
csv_Lab=csv_Lab.*(b+N_a);% the formula of our color space volume in Eq.(1)
...
% subplot(3, 3, 2); imshow(csv); title('csv');
% subplot(3, 3, 3); imshow(uint8(N_csv_Lab)); title('N_csv_Lab');
% subplot(3, 3, 4); imshow(uint8(a)); title('csv_Lab');
% subplot(3, 3, 5); imshow(uint8(b)); title('b');
% subplot(3, 3, 6); imshow(uint8(csv_refined)); title('csv_refined');
% subplot(3, 3, 7); imshow(uint8(only_obj_noise)); title('only_obj_noise');
% subplot(3, 3, 8); imshow(uint8(only_bj_noise)); title('only_bj_noise');
% subplot(3, 3, 9); imshow(uint8(gaussian_noise_img)); title('gaussian_noise_img');
end
